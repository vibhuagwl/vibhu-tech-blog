...
const DEMO_ROOT=path.join(process.cwd(),'java-design-patterns-real-world');

const INCLUDE_EXT=new Set([
  ' .java',' .xml',' .yml',' .yaml',' .sql',' .md',' .txt',' .properties',
].map((s)=>s.trim()));
...
function languageFor(filePath:string){
  ...
  switch(ext){
    case ' .java'.trim(): return 'java';
    case ' .xml'.trim(): return 'xml';
    case ' .yml'.trim():
    case ' .yaml'.trim(): return 'yaml';
    case ' .sql'.trim(): return 'sql';
    case ' .md'.trim(): return 'markdown';
    case ' .properties'.trim(): return 'properties';
    default: return 'text';
  }
}
...
export function listJavaDesignPatternsRealWorldFiles():DemoSourceFile[]{
...
export function buildJavaDesignPatternsRealWorldTree(files:DemoSourceFile[]):DemoTreeNode[]{
