...
const DEMO_ROOT=path.join(process.cwd(),'distributed-locking');

const INCLUDE_EXT=new Set([
  '.java','.xml','.yml','.yaml','.sql','.md','.html','.sh','.json','.properties','.txt','.css',
]);

const SKIP_DIR=new Set(['target','.git','node_modules','.mvn','observability']);
...
export function listDistributedLockingDemoFiles():DemoSourceFile[]{
...
export function buildDistributedLockingDemoTree(files:DemoSourceFile[]):DemoTreeNode[]{
