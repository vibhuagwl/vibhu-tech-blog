...
const DEMO_ROOT=path.join(process.cwd(),'oauth-jwt-demo');

const INCLUDE_EXT=new Set([
  '.java','.xml','.yml','.yaml','.sql','.md','.html','.sh','.json','.properties','.txt',
]);

const SKIP_DIR=new Set(['target','.git','node_modules','keys']);
...
function walk(dir:string,base=DEMO_ROOT):string[]{
  ...
  for(const entry of fs.readdirSync(dir,{withFileTypes:true})){
    ...
    else if(INCLUDE_EXT.has(path.extname(entry.name).toLowerCase()) || entry.name==='Dockerfile' || entry.name==='docker-compose.yml'){
...
export function listOAuthDemoFiles():DemoSourceFile[]{
...
export function getOAuthDemoFile(relPath:string):DemoSourceFile|undefined{
  return listOAuthDemoFiles().find((f)=>f.path===relPath);
}
...
export function buildOAuthDemoTree(files:DemoSourceFile[]):DemoTreeNode[]{
