...
export const SPOKEN = {
  "sixtySec": "CAP applies during a network partition in distributed shared-data systems. Consistency means linearizability — reads return the latest write or an error. Availability means every non-failing node responds successfully, even if stale. Partition tolerance is mandatory in real deployments. You cannot have both C and A while partitioned. I map business invariants to CP paths (ledger, inventory) and AP paths (feeds, DNS). I cite PACELC for latency vs consistency when healthy. I never stamp a product permanently CP or AP — I name the consistency level and config.",
  "twoMin": "Start by defining C, A, and P precisely — CAP C is not ACID C, and CAP A is not SLA uptime. Assume partition tolerance for any multi-node or multi-AZ system. The real question is: when nodes cannot communicate, do we reject operations that cannot be ordered with the majority, or do we respond with possibly stale data? For payments and inventory I choose CP: quorum writes, leader reads, idempotency for safe retries. For social feeds and CDN I accept AP with session read-your-writes and conflict resolution on heal. Technologies are tunable: Cassandra QUORUM is CP, ONE is AP; Mongo w:majority is CP; Kafka acks=all with minISR is CP on produce. I design hybrid systems with explicit per-operation contracts, observability on replication lag and quorum failures, and recovery plans for split-brain using fencing tokens and anti-entropy repair.",
  "staff": "Staff answers connect CAP to business invariants, failure domains, and operational evidence. I walk partition timelines: write on majority, minority rejects or serves stale, heal with read repair and merge policy. I distinguish linearizability, serializability, and session guarantees. I explain why sagas and outbox beat 2PC across microservices, and why exactly-once is end-to-end idempotency not broker magic. I critique labels — \"Mongo is CP\" is wrong without writeConcern. I use PACELC for multi-region latency. I discuss Jepsen-style validation, RPO/RTO vs async/sync replication, and when to sacrifice availability (return 503) versus accept staleness. Principal depth: prove intuition from Gilbert/Lynch, asymmetric partitions, fencing stale leaders, and per-request routing that implements CAP at the API layer with ETag/OCC and consistency query params."
};
