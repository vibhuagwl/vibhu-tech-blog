...
const DEMO_ROOT = path.join(process.cwd(), 'hadron-cashlines-dlq');

const INCLUDE_EXT = new Set([
  '.java', '.xml', '.yml', '.yaml', '.sql', '.md', '.html', '.sh', '.json', '.properties', '.txt', '.css',
]);

const SKIP_DIR = new Set(['target', '.git', 'node_modules', '.idea', 'build']);
...
export function listHadronDlqLabFiles(): DemoSourceFile[] {
...
export function buildHadronDlqLabTree(files: DemoSourceFile[]): DemoTreeNode[] {
