...
const DEMO_ROOT = path.join(process.cwd(), 'camunda-payment-platform');

const INCLUDE_EXT = new Set([
  '.java', '.xml', '.bpmn', '.yml', '.yaml', '.sql', '.md', '.html', '.sh', '.json', '.properties', '.txt', '.css',
]);

const SKIP_DIR = new Set(['target', '.git', 'node_modules', '.idea', '.gradle', 'build']);
...
export function listSpringCamundaLabFiles(): DemoSourceFile[] {
...
export function buildSpringCamundaLabTree(files: DemoSourceFile[]): DemoTreeNode[] {
