...
const DEMO_ROOT=path.join(process.cwd(),'spring-secrets-pii-demo');
...
function walk(dir:string,base=DEMO_ROOT):string[]{
  ...
  for(const entry of fs.readdirSync(dir,{withFileTypes:true})){
    ...
    else if(INCLUDE_EXT.has(path.extname(entry.name).toLowerCase()) || entry.name==='Dockerfile'){
...
export function listSpringSecretsPiiDemoFiles():DemoSourceFile[]{
...
export function buildSpringSecretsPiiDemoTree(files:DemoSourceFile[]):DemoTreeNode[]{
