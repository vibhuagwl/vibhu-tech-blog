...
export const SPOKEN = {
  thirtySec:
    'I do not choose REST or Kafka by preference. If the caller needs an immediate business answer — account validate, card auth, hard fraud stop — I use synchronous REST or gRPC with timeouts, circuit breaker, bulkhead, and idempotency. If the work is fan-out after commit — ledger, notify, reporting — I publish via transactional outbox to Kafka with idempotent consumers and DLQ. Production payment platforms are hybrid. Shared DB is not a communication mechanism.',
  twoMin:
    'Start from the business edge. Payment → Account validate is REST because checkout needs allow/deny in the same request. After accept, Payment commits and emits payment-created; Ledger, Notify, Fraud, and Reporting consume independently so Notify downtime does not fail money acceptance. gRPC is for tight internal RPC like risk scoring. Feign/RestClient are HTTP clients — they still need explicit timeouts and Resilience4j; WebClient only when the stack is truly non-blocking. Gateways do TLS, auth, rate limit — not domain orchestration. On failure: timeout is ambiguous so retries require Idempotency-Key; Kafka redelivery is normal so consumers upsert by paymentId; lag and DLQ are SLIs. I never claim broker exactly-once equals business exactly-once.',
  fiveMinStaff:
    'Draw the hybrid payment platform: Client → API Gateway → Payment Service. Sync REST to Account (and maybe gRPC to Risk) with connect/read timeouts, per-dependency CB and bulkhead, Idempotency-Key on money POSTs. Local commit plus outbox → Kafka payment-created keyed by paymentId → Ledger, Notification, Fraud enrich, Reporting — each consumer group idempotent with retry then DLQ. Expose payment status so clients see ACCEPTED vs LEDGER_POSTED under eventual consistency. Saga/compensations if capture fails after hold — do not unwind money because email failed. Observe RED on sync deps, consumer lag, CB open, DLQ depth, idempotency hits, end-to-end trace across HTTP and Kafka headers. Security: JWT at edge, mTLS or client-credentials east-west, Kafka ACLs, no PAN in logs. Contrast the bad design — five sync REST calls on the request thread — and name Kafka’s new costs: lag ops, status UX, outbox. Close with TRICKS-OLD and game-day drills.',
};
