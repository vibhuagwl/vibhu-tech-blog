...
/**
 * Redis / in-memory key for one bucket. Hash-tag the tenant so related keys can share a cluster
 * slot if a future multi-key Lua script is needed.
 *
  ...
  public static RateLimitKey from(RateLimitPolicy policy, RequestContext ctx) {
    ...
    String id =
        switch (policy.scope()) {
          ...
          case CLIENT ->
              hashedTenant + ":CLIENT:" + sanitize(ctx.require(ctx.clientId(), "clientId"));
          ...
          case SERVICE ->
              hashedTenant + ":SERVICE:" + sanitize(ctx.require(ctx.serviceName(), "serviceName"));
          case CLIENT_API ->
              hashedTenant
                  + ":CLIENT_API:"
          ...
          case TENANT_API ->
              hashedTenant + ":TENANT_API:" + sanitize(ctx.require(ctx.apiPath(), "apiPath"));
        };
