...
/**
 * Factory + flyweight: one {@link TokenBucketRateLimiter} per policy id. Strategy pattern:
 * algorithm enum selects the implementation. This lab's production path is token bucket; other
 * algorithms are rejected so interviews stay honest about what is actually implemented.
 */
public final class RateLimiterFactory {
  ...
  public RateLimiterFactory(
      RateLimitStore primary, InMemoryRateLimitStore localFallback, RateLimitMetrics metrics) {
  ...
  private RateLimiter create(RateLimitPolicy policy) {
    if (policy.algorithm() != RateLimitAlgorithm.TOKEN_BUCKET) {
      throw new UnsupportedOperationException(
          "Algorithm "
              + policy.algorithm()
              + " is documented in the hub but not implemented in this lab. Use TOKEN_BUCKET.");
    }
