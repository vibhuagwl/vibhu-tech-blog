...
/**
 * Pure token-bucket math shared by the in-memory store and documented as the contract the Redis Lua
 * script must match. Keep this lock-free and allocation-light.
 */
public final class TokenBucketMath {
  ...
  public static Decision consume(
      TokenBucketState current, RateLimitPolicy policy, long nowMs, double cost) {
    if (policy.blocked()) {
      return new Decision(
          false, 0, Duration.ofHours(1), policy.capacity(), TokenBucketState.full(0, nowMs));
    }
    ...
    if (tokens >= cost) {
      ...
      long retryMs =
          (long)
              Math.ceil((needed / (double) policy.refillRate()) * policy.refillPeriod().toMillis());
