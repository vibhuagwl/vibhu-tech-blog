...
public class WhatsAppMessageService {
  ...
  public UserView registerUser(RegisterUserRequest request) {
    ...
    return userRepository
        .save(new UserRecord(request.userId(), request.displayName(), request.phone()))
        .toView();
  }
  ...
  public ConversationView createDirectConversation(CreateDirectConversationRequest request) {
    ...
    if (request.userA().equals(request.userB())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Direct conversation needs two different users");
    }
  ...
  public MessageView sendMessage(
    ...
    if (!conversation.includes(senderId) || !conversation.includes(request.recipientId())) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "Sender and recipient must belong to conversation");
    }
    if (senderId.equals(request.recipientId())) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Sender and recipient must be different");
    }

    MessagePersistResult persistResult =
        messageRepository.saveIfAbsent(
            conversationId,
            senderId,
            request.recipientId(),
            clientMsgId,
  ...
  public List<MessageView> syncMessages(String conversationId, String requesterId, long afterSeq) {
    ...
    if (requesterId != null && !requesterId.isBlank() && !conversation.includes(requesterId)) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "Requester must belong to conversation");
    }
  ...
  public AckView acknowledge(String serverMsgId, AckRequest request) {
    ...
    MessageRecord message =
        messageRepository
            .findById(serverMsgId)
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Message not found"));
    if (!message.recipientId().equals(request.userId())
        && !message.senderId().equals(request.userId())) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "ACK user must be a message participant");
    }
    AckRecord ack =
        ackRepository.save(serverMsgId, request.userId(), request.deviceId(), request.type());
    return ack.toView();
  }
  ...
  public PresenceView heartbeat(PresenceHeartbeatRequest request) {
    ...
    localDeliveryCoordinator.ifAvailable(
        coordinator -> coordinator.deliverPendingFor(request.userId()));
    return presence;
  }
