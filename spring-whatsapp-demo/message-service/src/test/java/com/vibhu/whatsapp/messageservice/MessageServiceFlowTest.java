...
class MessageServiceFlowTest {
  ...
  void sendsMessageIdempotentlyAndSyncsByConversationSequence() {
    ResponseEntity<UserView> alice =
        restTemplate.postForEntity(
            "/api/v1/users",
            new RegisterUserRequest("alice", "Alice", "+15550001"),
            UserView.class);
    ResponseEntity<UserView> bob =
        restTemplate.postForEntity(
            "/api/v1/users", new RegisterUserRequest("bob", "Bob", "+15550002"), UserView.class);
    assertThat(alice.getStatusCode()).isEqualTo(HttpStatus.OK);
    assertThat(bob.getStatusCode()).isEqualTo(HttpStatus.OK);

    ResponseEntity<ConversationView> conversationResponse =
        restTemplate.postForEntity(
    ...
    SendMessageRequest sendRequest =
        new SendMessageRequest("client-msg-1", "bob", "opaque-ciphertext");

    ResponseEntity<MessageView> firstSend =
    ...
    ResponseEntity<MessageView> retrySend =
        restTemplate.exchange(
            "/api/v1/conversations/{conversationId}/messages",
            HttpMethod.POST,
            new HttpEntity<>(sendRequest, sendHeaders),
    ...
    ResponseEntity<List<MessageView>> syncResponse =
        restTemplate.exchange(
