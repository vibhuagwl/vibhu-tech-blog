...
public final class ClientCredentialsOAuthBearerToken implements OAuthBearerToken {
    ...
    public ClientCredentialsOAuthBearerToken(String value, String principalName, long lifetimeMs, Long startTimeMs,
            Set<String> scope) {
        this.value = value;
    ...
    public static ClientCredentialsOAuthBearerToken parse(String jwt) {
        ...
        long exp = payload.path("exp")
                .asLong(0) * 1000L;
        long iat = payload.path("iat")
                .asLong(0) * 1000L;
        Set<String> scopes = new LinkedHashSet<>();
        if (payload.has("scope") && payload.get("scope")
                .isTextual()) {
            Collections.addAll(scopes,
                    payload.get("scope")
                            .asText()
                            .split("\\s+"));
        }
