...
/**
 * API Gateway aggregation — fan-out parallel downstream calls with CompletableFuture. Maps to
 * curriculum Part 02 (API Gateway · BFF · aggregation).
 */
public final class AggregationService {
  ...
  public record OrderDashboard(
      String orderId, String customerName, String paymentStatus, int inventoryReserved) {}
  ...
  public CompletableFuture<OrderDashboard> aggregateOrderView(
    ...
    CompletableFuture<String> customerFuture =
        CompletableFuture.supplyAsync(customerLookup, executor);
    CompletableFuture<String> paymentFuture =
        CompletableFuture.supplyAsync(paymentLookup, executor);
    CompletableFuture<Integer> inventoryFuture =
        CompletableFuture.supplyAsync(inventoryLookup, executor);

    return CompletableFuture.allOf(customerFuture, paymentFuture, inventoryFuture)
        .thenApply(
            ignored ->
                new OrderDashboard(
                    orderId, customerFuture.join(), paymentFuture.join(), inventoryFuture.join()));
  }

  public <T> CompletableFuture<List<T>> parallelFetch(List<Supplier<T>> suppliers) {
    List<CompletableFuture<T>> futures =
        suppliers.stream().map(s -> CompletableFuture.supplyAsync(s, executor)).toList();
