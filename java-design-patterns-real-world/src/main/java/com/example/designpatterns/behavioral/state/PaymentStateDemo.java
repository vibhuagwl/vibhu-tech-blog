 ...
 * is becoming unsafe.
 *
 * <p>JAVA IMPLEMENTATION RULES 1. State interface methods represent events/operations; each
 * concrete state implements allowed transitions. 2. Context stores current State and delegates;
 * states call back to context to change state. 3. Illegal transitions must throw or return a domain
 * error — never silently no-op without reason. 4. Keep state objects typically stateless (or
 * flyweight); persist status on the context/entity. 5. Prefer enum + state objects over stringly
 * status codes scattered across the codebase.
 *
 * <p>DO NOT USE WHEN - Behavior does not depend on lifecycle — Strategy (choose algorithm) is the
 * better model.
 */
public class PaymentStateDemo {
  ...
  public static final class AuthorizedState implements PaymentState {
    ...
    public PaymentState settle() {
      throw new IllegalStateException("capture first");
    }

    public PaymentState complete() {
      throw new IllegalStateException("settle first");
    }
  ...
  public static final class SettledState implements PaymentState {
    ...
    public PaymentState fail(String reason) {
      throw new IllegalStateException("cannot fail after settlement");
    }
  ...
  public static final class FailedState implements PaymentState {
    ...
    public PaymentState fail(String ignored) {
