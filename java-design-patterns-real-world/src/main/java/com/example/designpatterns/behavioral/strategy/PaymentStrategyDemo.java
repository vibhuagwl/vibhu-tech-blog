 ...
 *
 * WHEN TO IMPLEMENT
 * - Multiple interchangeable algorithms/policies (payment rails, pricing, sorting) selected at runtime.
 * - You must eliminate sprawling switch/if on type codes inside one service method.
 *
 * JAVA IMPLEMENTATION RULES
 * 1. Define a Strategy interface with a clear method contract; each algorithm is a separate class.
 * 2. Context holds Strategy (or resolves via map/enum registry); context does not embed algorithm branches.
 * 3. Strategies should be stateless or carefully scoped — prefer pure functions of the request.
 * 4. Register strategies in a map for O(1) dispatch; fail on unknown keys.
 * 5. Open for extension: add a new strategy class without editing existing ones (OCP).
 *
 * DO NOT USE WHEN
 * - There is one algorithm, or State is a better fit because behavior depends on lifecycle transitions.
 */
