 ...
 *
 * WHEN TO IMPLEMENT
 * - Algorithm skeleton is fixed (validate → authorize → capture → notify) but steps vary by subclass.
 * - You need to enforce sequencing while allowing hooks.
 *
 * JAVA IMPLEMENTATION RULES
 * 1. Abstract class defines {@code final} template method calling overridable steps in order.
 * 2. Mark invariant steps final; mark extension points protected abstract or hook methods with defaults.
 * 3. Subclasses override only hooks — never rewrite the skeleton order.
 * 4. Prefer composition (Strategy) if variation is large and inheritance depth grows.
 * 5. Avoid public overridable methods that break the template contract.
 *
 * DO NOT USE WHEN
 * - Steps are independently swappable policies — Strategy/Chain may be clearer than inheritance.
 */
