 ...
 *
 * <p>JAVA IMPLEMENTATION RULES 1. Facade exposes a small, intention-revealing API; internally
 * coordinates subsystem types. 2. Do not put all business rules inside the facade — orchestrate;
 * keep domain logic in collaborators. 3. Prefer constructor injection of dependencies for testing.
 * 4. Map subsystem failures to one coherent application-level error model. 5. Facades may be
 * stateful for a use-case session, but avoid becoming a singleton god service.
 *
 * <p>DO NOT USE WHEN - There is only one class behind the call — no subsystem complexity to hide.
 */
public class PaymentFacadeDemo {
  public record PaymentOutcome(String status, String reference, List<String> steps) {}

  public static final class FraudService {
    boolean ok(int amount) {
      return amount < 5000;
    }
  }

  public static final class AccountService {
    boolean hasBalance(String account) {
      return !account.isBlank();
    }
  }

  public static final class PaymentService {
    String charge(String account, int amount) {
      return "charged:" + account + ":" + amount;
    }
  }

  public static final class NotificationService {
    String notifyCustomer(String account) {
      return "notified:" + account;
    }
  }

  public static final class AuditService {
    String audit(String account) {
  ...
  public static final class PaymentFacade {
    ...
    public String processPayment(String accountId, int amount) {
      return processDetailed(accountId, amount).status();
    }
