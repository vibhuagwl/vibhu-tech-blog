 ...
 *
 * WHEN TO IMPLEMENT
 * - You need a stand-in for access control, caching, lazy init, or remote call — same interface as the real subject.
 * - Clients must not know whether they talk to local, remote, or guarded implementations.
 *
 * JAVA IMPLEMENTATION RULES
 * 1. Proxy and RealSubject implement the same Subject interface.
 * 2. Proxy decides whether/when to delegate (authz, cache hit, lazy create).
 * 3. Do not reimplement core business logic in the proxy — only control access / lifecycle / I/O.
 * 4. For virtual proxy, synchronize lazy initialization carefully.
 * 5. Distinguish Proxy (control access) from Decorator (add behavior) and Adapter (convert interface).
 *
 * DO NOT USE WHEN
 * - You only need to add behavior freely — use Decorator; or interface mismatch — use Adapter.
 */
