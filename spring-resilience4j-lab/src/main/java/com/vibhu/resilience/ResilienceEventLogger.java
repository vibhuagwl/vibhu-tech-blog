...
public class ResilienceEventLogger {
    ...
    public ResilienceEventLogger(CircuitBreakerRegistry circuitBreakers, RetryRegistry retries) {
        circuitBreakers.getEventPublisher()
                .onEntryAdded(added -> subscribe(added.getAddedEntry()));
        circuitBreakers.getAllCircuitBreakers()
                .forEach(this::subscribe);
        retries.getEventPublisher()
                .onEntryAdded(added -> added.getAddedEntry()
                        .getEventPublisher()
                        .onRetry(event -> log.info("RETRY name={} attempt={}",
                                event.getName(),
    ...
    private void subscribe(CircuitBreaker breaker) {
        breaker.getEventPublisher()
                .onStateTransition(event -> log.info("CIRCUIT_{} name={} {}→{}",
                        ...
                        event.getStateTransition()
                                .getToState()))
                .onCallNotPermitted(event -> log.warn("CIRCUIT_OPEN_REJECT name={}", event.getCircuitBreakerName()));
    }
}
