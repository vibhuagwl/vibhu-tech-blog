...
public final class ProgrammaticDecorators {
    ...
    public static Supplier<PaymentResult> paymentPipeline(Supplier<PaymentResult> bankCall) {
        Bulkhead bulkhead = Bulkhead.of("payBh",
                BulkheadConfig.custom()
                        .maxConcurrentCalls(20)
                        .maxWaitDuration(Duration.ZERO)
                        .build());
        RateLimiter rateLimiter = RateLimiter.of("payRl",
                RateLimiterConfig.custom()
                        .limitForPeriod(50)
                        .limitRefreshPeriod(Duration.ofSeconds(1))
                        .timeoutDuration(Duration.ZERO)
                        .build());
        CircuitBreaker circuitBreaker = CircuitBreaker.of("payCb",
