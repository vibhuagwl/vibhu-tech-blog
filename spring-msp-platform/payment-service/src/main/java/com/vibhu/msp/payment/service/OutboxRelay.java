  ...
  public void relayPending() {
    for (OutboxEntity pending : outboxService.findPending()) {
      try {
        ...
        kafkaTemplate.send(MspTopics.PAYMENT_EVENTS, pending.getAggregateId(), payload);
        outboxService.markPublished(pending.getId());
        log.debug("Payment outbox published id={}", pending.getId());
      } catch (Exception ex) {
        log.error("Failed to publish payment outbox id={}: {}", pending.getId(), ex.getMessage());
      }
    }
  }
}
