...
class OrderServiceUnitTest {
  ...
  void idempotencyReturnsSameOrder() {
    ...
    OrderService service = new OrderService(
        orderRepository, orderLineRepository, outboxService,
        paymentClient, objectMapper
    );

    OrderEntity result = service.createOrder("key-1", "cust-1",
        List.of(new CreateOrderLine("SKU-1", 1, BigDecimal.TEN)));
