...
class InventoryServiceUnitTest {
  ...
  void reservesStock() {
    ...
    Mockito.when(reservationRepository.findByOrderId("ord-1")).thenReturn(java.util.Optional.empty());

    InventoryService service = new InventoryService(stockRepository, reservationRepository, outboxService);
    OrderCreated order = new OrderCreated("ord-1", "cust-1", BigDecimal.TEN,
