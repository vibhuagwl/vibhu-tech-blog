...
import type {MultiTenantTopic} from '@/lib/multi-tenant/types';
import CodePanel from './code-panel';

type Tab = 'problem' | 'flow' | 'code' | 'failure' | 'interview';

export default function TopicPanel({t}: {t: MultiTenantTopic}) {
        ...
        <div className="space-y-3 border-t border-slate-200 px-5 py-4 dark:border-slate-800">
          ...
          {tab === 'problem' && (
            <div className="space-y-3 text-sm leading-7 text-slate-600 dark:text-slate-300">
              ...
              <p>
                <span className="font-semibold text-slate-900 dark:text-white">Trade-off: </span>
                {t.tradeoff}
              </p>
            </div>
          )}
          ...
          {tab === 'code' && <CodePanel title="Production Java / Spring" code={t.code} tone="ok" />}
