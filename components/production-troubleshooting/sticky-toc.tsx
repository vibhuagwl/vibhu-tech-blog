...
import type {TocItem} from '@/lib/production-troubleshooting/types';
...
export default function StickyToc({items}:{items:TocItem[]}){
  ...
  return (
    <nav aria-label="Production troubleshooting sections" className="hidden xl:block">
      <div className="sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/95">
        <div className="text-[11px] font-semibold uppercase tracking-[.14em] text-slate-500">Incident · Stack · RCA</div>
        <input
          value={query}
