...
import type {ProdTopic} from '@/lib/production-troubleshooting/types';
import CodePanel from './code-panel';

type Tab='problem'|'flow'|'code'|'failure'|'interview';

export default function TopicPanel({t}:{t:ProdTopic}){
          ...
          <div className="flex flex-wrap gap-2">
            {([
              ...
              <button key={id} type="button" onClick={()=>setTab(id)}
                className={`rounded-md px-3 py-1.5 text-xs font-bold uppercase tracking-wide ${tab===id?'bg-slate-900 text-white':'bg-slate-100 text-slate-600 dark:bg-slate-900 dark:text-slate-300'}`}>
                {label}
              </button>
            ))}
          </div>
