...
import type {TocItem} from '@/lib/aws-interview/types';
...
export default function StickyToc({items}: {items: TocItem[]}) {
  ...
  const groups = [...new Set(filtered.map((i) => i.group).filter(Boolean))];

  return (
    <nav aria-label="AWS interview sections" className="hidden xl:block">
      <div className="sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950">
        <div className="text-[11px] font-semibold uppercase tracking-[.14em] text-slate-500">AWS Interview</div>
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Filter…"
          className="mt-3 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none ring-orange-500 focus:ring-2 dark:border-slate-700 dark:bg-slate-900"
        />
        <ul className="mt-3 space-y-3">
          {groups.length > 0
            ? groups.map((group) => (
                <li key={group}>
                  <div className="mb-1 text-[10px] font-bold uppercase tracking-[.1em] text-slate-400">{group}</div>
                        ...
                        <li key={item.id}>
                          <a
                            ...
                            className={[
                              ...
                              active === item.id
                                ? 'border-orange-600 font-semibold text-slate-900 dark:text-white'
