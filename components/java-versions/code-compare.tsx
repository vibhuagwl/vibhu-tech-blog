  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800">
          ...
          {(['compare','old','new'] as const).map((m)=>(
            <button
              ...
              className={[
                ...
                mode===m?'bg-blue-600 text-white':'bg-white text-slate-600 dark:bg-slate-950 dark:text-slate-300',
              ].join(' ')}
            >
      ...
      <div className={`grid gap-0 ${mode==='compare'?'md:grid-cols-2':''}`}>
        {(mode==='compare'||mode==='old') && (
          <div className="border-b border-slate-800 bg-slate-950 md:border-b-0 md:border-r">
            ...
            <pre className="overflow-x-auto p-4 text-[.82rem] leading-relaxed text-slate-100">{pair.old}</pre>
          </div>
        )}
        {(mode==='compare'||mode==='new') && (
          <div className="bg-slate-950">
            ...
            <pre className="overflow-x-auto p-4 text-[.82rem] leading-relaxed text-slate-100">{pair.new}</pre>
          </div>
        )}
      </div>
      ...
      <div className="grid gap-3 border-t border-slate-200 bg-white p-4 text-sm leading-6 text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300 md:grid-cols-2">
        <p><span className="font-semibold text-slate-900 dark:text-white">What changed?</span> {pair.whatChanged}</p>
        <p><span className="font-semibold text-slate-900 dark:text-white">Why?</span> {pair.why}</p>
        <p><span className="font-semibold text-slate-900 dark:text-white">Workload</span> {pair.workload}</p>
        <p><span className="font-semibold text-slate-900 dark:text-white">New bottleneck</span> {pair.newBottleneck}</p>
      </div>
    </div>
  );
}
