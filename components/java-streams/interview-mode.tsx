...
import {ALL, ARCHITECT, RAPID, SENIOR, STAFF} from '@/lib/java-streams/interview';

export default function InterviewMode() {
  const [mode, setMode] = useState<'senior' | 'staff' | 'architect' | 'rapid'>('senior');
  const list = useMemo(() => {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
      <div className="flex flex-wrap gap-2">
        {(
          [
            ['senior', 'Senior', SENIOR.length],
            ['staff', 'Staff', STAFF.length],
            ['architect', 'Architect', ARCHITECT.length],
            ['rapid', 'Rapid', RAPID.length],
          ] as const
        ).map(([id, label, n]) => (
          <button
            key={id}
            ...
            <div className="mt-4 space-y-3 text-sm leading-7 text-slate-700 dark:text-slate-300">
              ...
              {q.seniorInsight && (
                <p className="text-emerald-800 dark:text-emerald-300">
                  <strong>Senior:</strong> {q.seniorInsight}
                </p>
              )}
            </div>
          )}
          <p className="mt-3 text-xs text-slate-400">
            {idx + 1}/{list.length} · bank {ALL.length}
          </p>
        </>
      )}
    </div>
