...
export default function StoryWalkthrough() {
  ...
  const [storyId, setStoryId] = useState(SA_STORIES[0].id);
  const [beatId, setBeatId] = useState(WHITEBOARD_BEATS[0].id);
  const story = SA_STORIES.find((s) => s.id === storyId) ?? SA_STORIES[0];
  const beat = WHITEBOARD_BEATS.find((b) => b.id === beatId) ?? WHITEBOARD_BEATS[0];
  const storyIdx = SA_STORIES.findIndex((s) => s.id === story.id);
      ...
      <div className="flex flex-wrap gap-2">
        {(
          [
            ...
            ['whiteboard', '@Transactional whiteboard'],
          ] as const
        ).map(([id, label]) => (
          <button
            key={id}
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-950">
            ...
            <div className="mt-4 flex gap-2">
              <button
                type="button"
                onClick={() => setStoryId(SA_STORIES[(storyIdx - 1 + SA_STORIES.length) % SA_STORIES.length].id)}
              ...
              <button
                type="button"
                onClick={() => setStoryId(SA_STORIES[(storyIdx + 1) % SA_STORIES.length].id)}
                className="rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white"
              >
                Next
              </button>
              <span className="self-center text-xs text-slate-400">
                {storyIdx + 1}/{SA_STORIES.length}
              </span>
            </div>
          </div>
        </div>
