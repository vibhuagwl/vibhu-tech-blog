...
import {ALL, ARCHITECT, RAPID, SENIOR} from '@/lib/kafka-infra/interview';
import type {InterviewQ} from '@/lib/kafka-infra/types';

type DisplayQ = InterviewQ & {trick?: string};

export default function InterviewMode() {
  const [mode, setMode] = useState<'senior' | 'architect' | 'rapid'>('senior');
  const list = useMemo((): DisplayQ[] => {
        ...
        <>
          ...
          {revealed && (
            <div className="mt-4 space-y-3 text-sm leading-7 text-slate-700 dark:text-slate-300">
              ...
              {q.tradeoffs && (
                <p>
                  <strong>Tradeoffs:</strong> {q.tradeoffs}
                </p>
              )}
              {q.mistakes.length > 0 && (
                <p>
