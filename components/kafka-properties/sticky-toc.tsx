...
import type {TocItem} from '@/lib/kafka-properties/types';
...
export default function StickyToc({items}: {items: TocItem[]}) {
  ...
  return (
    <nav aria-label="Kafka properties sections" className="hidden xl:block">
      <div className="sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950">
        ...
        <input
          ...
          className="mt-3 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none ring-slate-400 focus:ring-2 dark:border-slate-700 dark:bg-slate-900"
        />
            ...
            <li key={item.id}>
              <a
                ...
                className={[
                  ...
                  active === item.id
                    ? 'border-slate-800 font-semibold text-slate-900 dark:border-slate-200 dark:text-white'
