...
public class ContextEngineeringService {
    ...
    public InvestigationContext build(ChatRequest request, UserContext user, Set<String> allowedTools) {
        ...
        for (Document doc : policyDocs) {
            String source = String.valueOf(doc.getMetadata()
                    .getOrDefault("source", "policy"));
            items.add(item("policy", source, filterPii(doc.getText()), ContextPriority.POLICY, user.tenantId()));
        }

        items.add(item("conversation",
                request.conversationId(),
                filterPii(request.message()),
                ContextPriority.CONVERSATION,
                user.tenantId()));

        items.add(item("memory",
                request.conversationId(),
        ...
        int used = budgeted.stream()
                .mapToInt(i -> i.content()
                        .length())
                .sum();
        ...
        return new InvestigationContext(request.conversationId(),
                user.tenantId(),
                user.userId(),
                paymentId,
                budgeted,
    ...
    public String renderSystemPrompt(InvestigationContext context) {
        ...
        sb.append("Allowed tools: ")
                .append(context.allowedTools())
                .append("\n");
        sb.append("Entities: ")
                .append(context.entities())
                .append("\n");
        return sb.toString();
    }
    ...
    private List<ContextItem> applyBudget(List<ContextItem> items, int budget) {
        ...
        for (ContextItem item : items) {
            ...
            result.add(new ContextItem(item.sourceType(),
                    item.sourceId(),
                    content,
                    item.priority(),
                    item.timestamp(),
    ...
    private static ContextItem item(String sourceType, String sourceId, String content, ContextPriority priority,
            String tenantId) {
        return new ContextItem(sourceType, sourceId, content, priority.weight(), Instant.now(), "HIGH", tenantId);
    }
    ...
    static String filterPii(String text) {
        ...
        String filtered = PII_EMAIL.matcher(text)
                .replaceAll("[REDACTED_EMAIL]");
        return PII_PHONE.matcher(filtered)
