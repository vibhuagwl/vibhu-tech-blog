...
public class ShardedCounterStore {
  ...
  public ShardKey chooseShard(String resourceId, String stableShardHint) {
    int shard =
        stableShardHint == null || stableShardHint.isBlank()
  ...
  public Map<Integer, Long> shardMap(String resourceId) {
    ...
    for (int shard = 0; shard < shardCount; shard++) {
      result.put(
          shard, counters.getOrDefault(new ShardKey(resourceId, shard), new AtomicLong()).get());
    }
    return result;
  }
  ...
  public List<ShardValue> shardValues(String resourceId) {
    ...
    shardMap(resourceId)
        .forEach(
            (shard, value) ->
                values.add(
                    new ShardValue(shard, new ShardKey(resourceId, shard).redisKey(), value)));
    return values;
  }
