...
@SpringBootTest(properties = {"spring.security.oauth2.resourceserver.jwt.issuer-uri=", "spring.security.oauth2.resourceserver.jwt.jwk-set-uri=https://idp.test/jwks"})
...
class ResourceApiSecurityTest {
    ...
    void userGroup_canReadPayments() throws Exception {
        mockMvc.perform(get("/api/payments").with(jwt().jwt(j -> j.claim("upn", "alice@corp.example")
    ...
    void userGroup_cannotReadAdmin() throws Exception {
        mockMvc.perform(get("/api/admin/stats").with(jwt().jwt(j -> j.claim("groups", List.of("App.Payments.Users")))
                        .authorities(new SimpleGrantedAuthority("ROLE_USER"))))
    ...
    void adminGroup_canReadAdmin() throws Exception {
        mockMvc.perform(get("/api/admin/stats").with(jwt().jwt(j -> j.claim("groups",
                                List.of("App.Payments.Users", "App.Payments.Admins")))
