...
class SettlementRepository {
    ...
    List<Map<String, Object>> processed() {
        return jdbcTemplate.queryForList("select payment_id, processed_at from processed_payments order by processed_at desc");
    }

    List<Map<String, Object>> dlq() {
        return jdbcTemplate.queryForList("select payment_id, reason, created_at from dlq_events order by created_at desc");
    }
}
...
class SettlementService {
    ...
    void publishResult(PaymentRequestedEvent event, String status, String detail) {
        kafkaTemplate.send(
                ...
                new PaymentResultEvent(event.paymentId(), event.accountId(), status, detail, Instant.now()));
    }
