...
public class RefreshTokenService {
  ...
  public RefreshTokenService(
      RefreshTokenRepository refreshTokenRepository, JwtProperties properties) {
  ...
  /**
   * Rotate: revoke the presented token, issue a new one in the same family. Reuse of a revoked
   * token revokes the entire family (theft detection).
   */
  @Transactional
  public IssuedRefreshToken rotate(String rawRefreshToken) {
    ...
    if (existing.isRevoked()) {
      ...
      log.warn(
          "Refresh token reuse detected; family revoked userId={}", existing.getUser().getId());
