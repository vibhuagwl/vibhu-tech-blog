 ...
 *
 * <p>Secret JSON expected: {@code {"hmac":"...at least 32 bytes..."}}
 * or a raw string of 32+ bytes.
 *
 * <p>In most ECS/EKS setups it is simpler (and better for rotation via task restart)
 * to inject {@code JWT_SECRET} as an environment variable from Secrets Manager
 * and keep {@code secret-source: env}. Use this bean only when the JVM must
 * pull the secret itself (for example, rotating without a redeploy).
 */
@Component
@Profile("aws-secrets")
