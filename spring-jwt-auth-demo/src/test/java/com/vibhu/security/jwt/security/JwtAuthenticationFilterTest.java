...
class JwtAuthenticationFilterTest {
    ...
    void invalidJwtWrites401() throws Exception {
        when(jwtService.extractUsername("bad")).thenThrow(new InvalidTokenException("Invalid access token"));
        MockHttpServletRequest request = new MockHttpServletRequest("GET", "/api/users/me");
        request.addHeader("Authorization", "Bearer bad");
        MockHttpServletResponse response = new MockHttpServletResponse();
        filter.doFilter(request, response, chain);
        verify(entryPoint).write(response, 401, "Unauthorized", "Invalid or expired access token", request);
        assertThat(SecurityContextHolder.getContext().getAuthentication()).isNull();
    }
}
