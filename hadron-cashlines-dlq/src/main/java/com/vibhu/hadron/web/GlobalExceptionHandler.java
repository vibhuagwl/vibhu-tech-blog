...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ErrorResponse> conflict(
      ReplayConflictException ex, HttpServletRequest request) {
    return ResponseEntity.status(HttpStatus.CONFLICT)
        .body(
            new ErrorResponse(
                "replay_conflict", ex.getMessage(), request.getHeader("X-Correlation-Id")));
  }
  ...
  public ResponseEntity<ErrorResponse> validation(Exception ex, HttpServletRequest request) {
    return ResponseEntity.badRequest()
        .body(
            new ErrorResponse(
                "validation_failed", ex.getMessage(), request.getHeader("X-Correlation-Id")));
  }
