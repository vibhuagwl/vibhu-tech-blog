...
public class LockController {
  ...
  public ResponseEntity<Map<String, Object>> renew(@RequestBody LockRenewRequest request) {
    boolean renewed =
        lockManager.renew(
            request.lockKey(), request.ownerToken(), durationFromMillis(request.leaseMs()));
    if (!renewed) {
      return ResponseEntity.status(409)
          .body(Map.of("renewed", false, "message", "Lock token is not held"));
    }
  ...
  public ResponseEntity<Map<String, Object>> release(@RequestBody LockReleaseRequest request) {
    ...
    if (!released) {
      return ResponseEntity.status(409)
          .body(Map.of("released", false, "message", "Lock token is not held"));
    }
